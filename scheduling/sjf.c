...
int main()
{
    ...
    {
        p[i].finished = 0;
        printf("Enter the process id of the process : ");
        scanf("%d", &p[i].pid);
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        for (int j = 1; j <= n; j++)
        {
            if (p[j].finished == 0 && p[j].at <= timeElapsed && p[j].bt <= cur)
            {
                cur = p[j].bt;
                nextprocess = p[j].pid;
            }
            if (p[j].finished == 0 && p[j].at < mn)
            {
                mn = p[j].at;
                nextid = p[j].pid;
            }
    ...
    printf("\nAvg waiting time : %f and Avg Turnaround time : %f \n", avgWaitTime / (n + 0.0), avgTurnAround / (n + 0.0));
}